...
% w_r: weight for the response surface criterion
%
% Output:
...
[CandValue, NormValue] = ComputeRBF(CandPoint,Data);  % 计算候选点的响应面函数值，以及候选点与已知点的距离矩阵
...
if NumberNewSamples == 1  % only one point is selected
    ...
    % points
    CandTotalValue(CandMinDist < Data.tolerance) = Inf;  % 候选点太接近已知点的话就舍去
...
% delete identical selected sample points
kk=1;  
while kk <= size(xselected,1)-1 
    jj=kk+1;
    while jj <= size(xselected,1)
        if sqrt(sum((xselected(kk,:)-xselected(jj,:)).^2)) < Data.tolerance
            xselected(jj,:)=[];  % 删除太接近的采样点
        else
            jj=jj+1;
        end
    end
