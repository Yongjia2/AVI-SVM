function Data = DYCORSrestart(Data,maxeval,NumberNewSamples)  
%----------------********************************--------------------------
...
%--------------------------------------------------------------------------
% DYCORSrestart is the surrogate model algorithm that uses an RBF
...
%                   required.(新采样点个数，如果 >1 则调用并行工具箱计算改点函数值)
%
% Output:
...
% m = 2*(Data.dim+1);  % number of points in initial experimental design （初始点个数）
m = 20;
% initialize arrays for collecting results of current trial
% number of restarts (when algorithm restarts within one trial after encountering local optimum)
numstart = 0;
...
while numevals < maxeval % do until max. number of allowed f-evals reached
    ...
    k = 1;
    while rank_P ~= Data.dim+1  % 满足系数矩阵可逆条件
        Data.S = SLHDstandard(Data.dim,m);  % 产生初始点（m为初始试验点个数）,采用对称拉丁超立方体设计
    ...
    Data= DYCORS_opt(Data, maxeval-numevals, NumberNewSamples);
