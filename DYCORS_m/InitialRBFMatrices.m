...
PHI = zeros(maxevals);  % maxevals × maxevals 的矩阵
switch Data.phifunction
    ...
    case 'thinplate'
        ...
        PairwiseDistance(logical(eye(size(PairwiseDistance)))) = 0;  % 防止PairwiseDistance为0时计算0**2*log(0)，设置相同点之间距离为0
end
...
case 'quadratic'
    pdim = ((Data.dim+1)*(Data.dim+2))/2;  % (dim+1)+(1+2+...+dim) = ((Data.dim+1)*(Data.dim+2))/2
